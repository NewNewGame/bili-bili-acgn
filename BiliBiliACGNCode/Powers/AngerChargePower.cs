...
public sealed class AngerChargePower : PowerBaseModel
{
    ...
    public override async Task AfterPowerAmountChanged(PlayerChoiceContext choiceContext, PowerModel power, decimal amount, Creature? applier, CardModel? cardSource)
    {
        ...
        if(base.Amount >= MAXCHARGE && power is AngerChargePower){
            ...
            await PowerCmd.Apply<AngerChargePower>(choiceContext, base.Owner, -MAXCHARGE * triggerCount, base.Owner, cardSource);
            while(triggerCount > 0){
                // 进入红怒，红怒是Single类型所以要一个个给。
                await PowerCmd.Apply<BerserkPower>(choiceContext, base.Owner, 1, base.Owner, cardSource);
                triggerCount--;
            }
        }
    }
